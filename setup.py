...
iface_mod = Extension(
    "netifaces",
    sources=[
        os.path.join(os.path.abspath(os.path.dirname(__file__)), "netifaces.c")
    ],
    libraries=libraries,
                ...
                if link:
                    ...
                    if execute:
                        abspath = os.path.abspath(
                            os.path.join(self.build_temp, "conftest")
                        )
            ...
            if result:
                ...
                for sockaddr in result:
                    self.compiler.define_macro(
                        "HAVE_SOCKADDR_%s" % sockaddr.upper(), 1
                    )
            else:
...
setup(
    ...
    cmdclass=({"bdist_wheel": _bdist_wheel, "build_ext": my_build_ext}),
)
